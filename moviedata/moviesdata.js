...
function displayMovies(moviesToDisplay, page = 1) {
    ...
    const paginatedMovies = moviesToDisplay.slice(startIdx, startIdx + MOVIES_PER_PAGE);
        ...
        const movieCard = `
          <div class="col-lg-3 col-md-4 col-sm-6 mb-4">
            <div class="movie-card">
              <img src="${movie.posterUrl}" alt="${movie.title}" class="movie-poster" onerror="this.onerror=null; this.src='https://b.rgbimg.com/users/h/hi/hisks/600/mhYExIC.jpg';">
              <div class="movie-body">
                ...
                <div class="movie-footer">
                  <span class="movie-actors">${movie.actors.split(',').slice(0, 2).join(',')}${movie.actors.split(',').length > 2 ? '...' : ''}</span>
                </div>
              </div>
            </div>
          </div>
...
// Function to initialize genre filters
...
async function init() {
    ...
    try {
        ...
        // Process data
        allGenres = moviesData.genres && moviesData.genres.length > 0
            ? moviesData.genres
...
$(document).ready(function () {
    ...
    $('.search-input').on('input', function () {
        ...
        filteredMovies = moviesData.movies.filter(movie =>
    ...
    $(document).on('click', '.page-link', function (e) {
        ...
        if (text === 'Previous' && currentPage > 1) {
            currentPage--;
        } else if (text === 'Next' && currentPage < Math.ceil(filteredMovies.length / MOVIES_PER_PAGE)) {
            currentPage++;
        } else if (!isNaN(text)) {
